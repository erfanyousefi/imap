...
export class StudentService {
    ...
    async create(studentDto: CreateStudentDto) {
        const { name: full_name, email, mobile } = studentDto;
    ...
    async findOneByMobile(mobile: string) {
        ...
        if (student) throw new NotFoundException(StudentMessage.AlreadyExistMobile);
        return true;
    }
    async findOneByEmail(email: string) {
        const student = await this.studentRepository.findOneBy({ email });
        if (student) throw new NotFoundException(StudentMessage.AlreadyExistEmail);
        return true;
    }
    async edit(id: string, studentDto: CreateStudentDto) {
